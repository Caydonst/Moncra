...
export function rollItemStats(itemType, itemRarity) {
    if (itemType === "Weapon") {
...
function randomBetween(min, max, decimals = 0) {
    ...
    const value = Math.floor(Math.random() * (scaledMax - scaledMin + 1)) + scaledMin;
    const rolledValue = value / multiplier;
    const percentage = ((rolledValue - min) / (max - min)) * 100;
    return {
        value: rolledValue,
        percentage: Math.round(percentage)
    };
}
export function getMaxStatValue(rarity, stat) {
...
export function getMinStatValue(rarity, stat) {
    const rarityMultiplier = multipliers[rarity];
    switch (stat) {
        case "damage":
            return Math.round(baseAttack * rarityMultiplier.min);
        case "hp":
            return Math.round(baseHp * rarityMultiplier.min);
        case "armor":
            return Math.round(baseArmor * rarityMultiplier.min);
        case "crit":
            return rarityMultiplier.crit.min;
    }
}
//# sourceMappingURL=itemRolls.js.map